...
if [ "$TRAVIS_REPO_SLUG" == "JoErNanO/icub-interactionforces" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_BRANCH" == "master" ] && [ "$TRAVIS_BUILD_TYPE" == "Release" ]; then
    ...
    git clone --quiet --branch=gh-pages https://${GH_TOKEN}@github.com/JoErNanO/icub-interactionforces gh-pages > /dev/null
